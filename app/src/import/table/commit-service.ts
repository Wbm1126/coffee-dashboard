...
export async function commitTablePreview(
  ...
  const validTargets = new Set(preview.items.map((item) => item.beanKey).concat(
    duplicateItems.flatMap((item) => (item.duplicateTargets ?? []).map((target) => target.beanKey)),
  ));
  for (const beanKey of Object.keys(duplicateDecisions)) {
    if (!validTargets.has(beanKey)) throw new InvalidTableResolutionError(`重复决策 ${beanKey} 不属于当前预览。`);
  }
  ...
  if (unresolvedSkips.length > 0) throw new UnresolvedTableDuplicateError(unresolvedSkips);
  ...
  const transaction = await repository.transact(preview.baseRevision, 'before-table-import', (draft) => {
    ...
    for (const item of preview.items) {
      ...
      // merge：并入目标豆，只填空字段，不覆盖已有值；import_as_new：作为新豆入库。
      ...
      for (const candidate of evaluations) {
        // 同豆同分的 legacy_import 评价不重复入库（batchKey 之外的额外防线）。
        const exists = draft.beanEvaluations.some(
