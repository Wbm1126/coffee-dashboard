  ...
  return <Dialog open={Boolean(item)} labelledBy="bean-detail-title" onClose={onClose} returnFocusTo={returnFocusTo} className="bean-drawer">
    {item && bean && <>
      ...
      <section aria-labelledby="bean-lastbrew-title"><h3 id="bean-lastbrew-title">最近冲煮</h3>{(() => {
        ...
        return <dl className="detail-facts"><Fact term="日期" value={lastWithParams.record.drankOn} /><Fact term="粉量" value={params.doseGrams === null ? null : `${params.doseGrams}g`} /><Fact term="液量" value={params.yieldGrams === null ? null : `${params.yieldGrams}g`} /><Fact term="水温" value={params.temperatureC === null ? null : `${params.temperatureC}°C`} /><Fact term="研磨" value={params.grindSetting} /><Fact term="水量" value={params.waterGrams === null ? null : `${params.waterGrams}g`} /><Fact term="奶量" value={params.milkGrams === null ? null : `${params.milkGrams}g`} /></dl>;
      })()}</section>
      ...
      <details className="provenance-details" aria-label="数据来源与出处">
        ...
        {(() => {
          ...
          return <>{sources.length > 0 && <p><b>商品来源：</b>{sources.map((source) => safeExternalUrl(source.url) ?? '不可打开的来源').join('；')}</p>}<ul>{fieldEntries.map(([field, entries]) => { const latest = (entries ?? [])[(entries ?? []).length - 1]!; return <li key={field}>{field}：{latest.sourceKind === 'user' ? '手工确认' : latest.sourceKind === 'excel' ? '历史表格' : latest.sourceKind === 'official' ? '官方页面' : '搜索结果'} · {latest.location || latest.sourceId} · {new Date(latest.capturedAt).toLocaleDateString('zh-CN')}</li>; })}</ul></>;
        })()}
      </details>
